...
| Registration identity                                                      | One application per musician + scheduled song slot + instrument; multiple instruments for the same slot are allowed.                                                        | Accepted. Require non-null `escalaId` and `instrumento` for new registrations, preflight legacy rows, then add canonical unique (`musicoId`, `escalaId`, `instrumento`). Instrument counts are accepted as guidance, not approval limits. |
